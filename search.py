...
def cornersHeuristic(state, problem):
    ...
    #make a list of unvisted corners
    unvisitedCorners = []
    for i in range(len(cornersUnvisited)):
        if not cornersUnvisited[i]:
            unvisitedCorners.append(corners[i])

    #calculate distance from current node to all corner nodes
    if len(unvisitedCorners) > 0:
        heuristic = findClosestPoint(currentLocation, unvisitedCorners)
    return heuristic
